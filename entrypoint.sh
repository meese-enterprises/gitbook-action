...
if [ "${INPUT_PUBLISH_COMMIT_HISTORY}" = "false" ]; then
  print_warning "3305-0: Pay attention, you have allowed push force"
  if [ "${INPUT_PUBLISH_PUSH_FORCE}" = "false" ]; then
    print_error "3108: You set publish_commit_history to false, you must set publish_push_force: to+true as well"
  fi
fi
...
if [ $? -eq 0 ]; then  # clone success
  ...
  if [ ${INPUT_SOURCE_FILE_WITHOUT_PREFIX} = "true" ] ; then
    ...
    # move file without name like .nojekyll, may cause some error
    cp -rfp local_source_temp/${INPUT_SOURCE_DIR}/. local_source
  else
    # move source with source2 to build together
    cp -rfp local_source_temp/${INPUT_SOURCE_DIR}/* local_source
  fi
  ...
  if [ $? -eq 0 ]; then
    ...
    # git clone success but the gitbook source dir set wrong
    print_error "3102: Source git clone success, but not find gitbook-source file in ${INPUT_SOURCE_DIR}"
  fi

else
  # can't git clone 1. no repo 2.no auth
  print_error "3103: Can't find this source_repo/branch, make sure you've added token or source_token (with access) in gitbook_action.yml"
fi
...
if [ ${INPUT_SOURCE2_REPO} != "null" ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    print_error "3103-2: Can't find this source2_repo/branch, make sure you've added token_source2 (with access) in gitbook_action.yml"
  fi
fi
...
if [ $? -eq 0 ]; then
  # git clone this repo.branch success
  print_info "Message: publish git success"
else
  # can't git clone this repo.branch try git clone this repo
  ...
  if [ $? -eq 0 ]; then # clone repo success, create new branch
    ...
    # can't git clone 1. no repo 2.no auth
    print_error "3104: Can't find this publish_repo, make sure you've added token or publish_token (with access) in gitbook_action.yml"
  fi
fi
...
if [ ${INPUT_PUBLISH2_REPO} != "null" ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      print_error "3104-2: Can't find this publish2_repo, make sure you've added publish2_token (with access) in gitbook_action.yml"
    fi
  fi
fi
...
if [ ${INPUT_PUBLISH3_REPO} != "null" ]; then
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      git commit -m "Initial commit"
      cd ..
      print_warning "3302-3: Can't find this publish3_branch, but find this repo, checking out a new branch"
    else
      print_error "3104-3: Can't find this publish3_repo, make sure you've added publish3_token (with access) in gitbook_action.yml"
    fi
  fi
fi
...
if [ $? -eq 0 ]; then
  ...
  print_warning "3305: Can't push publish_repo/branch, try push force"
    ...
    if [ $? -eq 0 ]; then
      ...
      print_error "3106: Can't push publish_repo/branch, make sure you've added publish_token (with access) in gitbook_action.yml"
    fi
else
  print_error "3107: Can't push publish_repo/branch, make sure you've added publish_token (with access) in gitbook_action.yml or try setting publish_push_force to true"
fi
cd ..
  ...
  if [ $? -eq 0 ]; then
    ...
    print_warning "3305-2: Can't push publish2_repo/branch, try push force"
      ...
      if [ $? -eq 0 ]; then
        ...
        print_error "3016-2: Can't push publish2_repo/branch, make sure you've added publish2_token (with access) in gitbook_action.yml"
      fi
  else
    print_error "3017-2: Can't push publish2_repo/branch, make sure you've added publish2_token (with access) in gitbook_action.yml or try setrint publish2_push_force to true"
  fi
  cd ..
fi
  ...
  if [ $? -eq 0 ]; then
    ...
    print_warning "3305-3: Can't push publish3_repo/branch, try push force"
      ...
      if [ $? -eq 0 ]; then
        ...
        print_error "3016-3: Can't push publish3_repo/branch, make sure you've added publish3_token (with access) in gitbook_action.yml"
      fi
  else
    print_error "3017-3: Can't push publish3_repo/branch, make sure you've added publish3_token (with access) in gitbook_action.yml or try setting publish3_push_force to true"
  fi
  cd ..
fi
...
if [ "${INPUT_NOT_CLEAN}" = "false" ]; then
  ...
  print_warning "3307: NOT_CLEAN set to true, if two gitbook-action in one workflow may run fatal. If don't use cache, (source build file-->node_modules), please set false(Default), and CLEAN is not remove gitbook-cli and gitbook"
fi
